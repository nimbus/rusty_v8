...
#[test]
fn detach_all_slots_nulls_annex_without_dropping() {
  // detach_all_slots() should null the annex pointer so get_slot returns
  // None, but NOT eagerly drop the annex — V8's GC weak finalizer handles
  // cleanup. This is used during reset_main_realm to avoid heap corruption.
  use std::cell::Cell;
...

  {
    ...
    context.set_slot(Rc::new(DropMarker(dropped.clone())));
    assert!(context.get_slot::<DropMarker>().is_some());

    // detach_all_slots nulls the pointer but does NOT drop the annex
    context.detach_all_slots();

    // Slot should no longer be readable
    assert!(
      context.get_slot::<DropMarker>().is_none(),
      "get_slot should return None after detach_all_slots"
    );

    // The DropMarker should NOT have been dropped yet — the annex is
    // still alive, held by the weak finalizer until GC collects the context
    assert!(
      !dropped.get(),
      "annex should not be eagerly dropped by detach_all_slots"
    );
  }
}
